...
> Note: `onion_logger` requires the `onion_datetime` target to already exist.
...
```bash
cmake -DONION_LOGGER_BUILD_TESTS=OFF ..
```
...
The build will fail if the CMake target `onion_datetime` is not available.
